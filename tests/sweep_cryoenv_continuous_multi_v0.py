...
for di in range(nmbr_detectors):
    for amp in TEST_PULSE_AMPLITUDES:
        ...
        for v_s in np.linspace(env_kwargs['dac_iv'][1], 
            ...
            action = np.array([dacs_i, [10]*nmbr_detectors]).flatten('F')  # wait = 10
            # print(f'action: {action}')
            new_state, _, _, _ = env._step(action=action)
            count += 1
        # import ipdb; ipdb.set_trace()
...
for i in range(nmbr_detectors):
    rewards, dV, wait, reset, new_dac, new_ph, T, T_inj = trajectories[0+3*i]  # low pulses
    ...
    for j, h, p1, p2 in zip(range(len(host)), host, par1, par2):
        ...
        color3 = 'grey'
        # ipdb.set_trace()
        p1p, =  h.plot(new_dac[:,i], rewards, color=color1, label="Reward", linewidth=1, zorder=10)
        p2p, = p1.plot(new_dac[:,i], new_ph[:,j], color=color2, label="Control Pulse Height", linewidth=1, marker='.', linestyle='', zorder=15)
        _    = p1.plot(new_dac[:,i], new_ph_mid[:,j], color=color2, linewidth=1, marker='.', linestyle='', zorder=15)
        _    = p1.plot(new_dac[:,i], new_ph_small[:,j], color=color2, linewidth=1, marker='.', linestyle='', zorder=15)
        p3p, = p2.plot(new_dac[:,i], T[:,j], color=color3, label="Simplified Temperature", linewidth=1, linestyle='dashed')
