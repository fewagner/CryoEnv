...
from cryoenv.buffers.replay_buffer import ReplayBuffer
...
class SAC_v2:
    def __init__(self, env, state_dim, action_dim,
        ...
        self.value = ValueNetwork(state_dim, lr=lr_crit_val,
                                  device=self.device, hidden_dims=hidden_dims)
        self.target_value = ValueNetwork(state_dim, lr=lr_crit_val, hidden_dims=hidden_dims,
                                         name='target_value', device=self.device)
    ...
    def learn(self, episodes, episode_steps):
        for ep in tqdm(range(episodes)):
            ...
            while not done and i < episode_steps:
                ...
                i += 1
                state = new_state
            log.info(f'episode: {ep}, score: {score}')
